//Copyright [2017-09-22] by [陈思悦、刘锦澄、冯怡宁、王一菲、沈凌儿] from [Class 2016215109 And 2016215110 from BUPT International School] 
//All rights reserved. 
//Version 5.0.
#include <stdio.h>
#include <stdlib.h>
...
void SearchItemName(){//商品名称搜索
    ...
    rewind(stdin);
    scanf("%s",name);
    printf("| 商品名 |商品编号|超市名称| 售价 |销量|库存|折扣价 |  折扣开始时间  |  折扣结束时间  |\n");
    for(i=0;i<ItemNum;i++){
...
void CARTFUNC(){
    ...
    {
        printf("\t***购物车***\n");
        printf("|商品编号| 商品名 | 超市名称 |  折扣开始时间  |  折扣结束时间  | 售价 |折扣价| 库存 |\n");
        cart=fopen("cart.txt","a+");
